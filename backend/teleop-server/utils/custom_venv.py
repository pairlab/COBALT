...
def _worker(
    ...
    try:
        while True:
            ...
            elif cmd == "regenerate_obs_from_state":
                p.send(env.regenerate_obs_from_state(data))
            elif cmd == "get_env_state":
                p.send(env.get_env_state())
            elif cmd == "set_env_state":
                p.send(env.set_env_state(data))
            else:
                p.close()
                raise NotImplementedError
...
class SubprocEnvWorker(EnvWorker):
    ...
    def regenerate_obs_from_state(self, mujoco_state):
        self.parent_remote.send(["regenerate_obs_from_state", mujoco_state])
        obs = self.parent_remote.recv()
        if self.share_memory:
            obs = self._decode_obs()
        return obs
    ...
    def get_env_state(self):
        self.parent_remote.send(["get_env_state", None])
        return self.parent_remote.recv()

    def set_env_state(self, env_state):
        self.parent_remote.send(["set_env_state", env_state])
        return self.parent_remote.recv()
...
class DummyVectorEnv(BaseVectorEnv):
    ...
    def regenerate_obs_from_state(self, mujoco_state):
        return [w.regenerate_obs_from_state(mujoco_state) for w in self.workers]

    def get_env_state(self):
        return [w.get_env_state() for w in self.workers]

    def set_env_state(self, env_state):
        return [w.set_env_state(env_state) for w in self.workers]

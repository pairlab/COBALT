...
with open(config_dir / "isaaclab_tasks.json", "r") as f:
    TASKS = json.load(f)


class BaseServerConfigIsaac(CCRConfig):
    def __init__(self, *args, **kwargs):
        super(BaseServerConfigIsaac, self).__init__(*args, **kwargs)

        # predictable object placements (don't use while collecting training data)
        self.robot.seeded_object_sampling = False

        self.simulator.name = "isaaclab"
        ...
        for key in ["left", "right"][: self.num_device]:
            self.robot.names[key] = "IsaacLabRobot"

        self.validate_action_dim()                                               # checks that all specified robots have the same action dims

        self.action_dim = ROBOTS_ACTION_DIM["IsaacLabRobot"] * self.num_device
